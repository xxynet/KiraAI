"""Supervisor: the process `python main.py` runs by default.

It spawns `python main.py --_child` and keeps it alive: exit code
RESTART_EXIT_CODE triggers a restart with exponential backoff (self-restart
after updates), while stop signals are forwarded so the child can shut down
gracefully within STOP_GRACE_SECONDS before it is killed.
"""
...
def run_supervisor(args: argparse.Namespace, script_dir: str):
