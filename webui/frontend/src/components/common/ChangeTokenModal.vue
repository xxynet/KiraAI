  ...
  <Modal v-model="visible" content-class="max-w-md">
    <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full flex flex-col" style="max-height: 90vh;">
      <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <div>
          <h3 class="text-lg font-semibold text-theme-strong">{{ $t('settings.token_title') }}</h3>
          <p class="mt-1 text-sm text-theme-subtle">{{ $t('settings.token_desc') }}</p>
        </div>
        <button type="button" class="text-theme-faint text-theme-faint-hover" @click="visible = false">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
      </div>
      <div class="px-6 py-4 flex-1 overflow-y-auto space-y-4">
        <div>
          <label for="modal-token-old" class="block text-sm font-medium text-theme-body mb-2">{{ $t('settings.token_old') }}</label>
          <UiInput
            id="modal-token-old"
            v-model="oldToken"
            type="password"
        ...
        <div>
          <label for="modal-token-new" class="block text-sm font-medium text-theme-body mb-2">{{ $t('settings.token_new') }}</label>
          <UiInput
            id="modal-token-new"
            v-model="newToken"
            type="password"
        ...
        <div>
          <label for="modal-token-confirm" class="block text-sm font-medium text-theme-body mb-2">{{ $t('settings.token_confirm') }}</label>
          <UiInput
      ...
      <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-3">
        <button type="button" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-theme-body hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" @click="visible = false">{{ $t('settings.backup_confirm_cancel') }}</button>
